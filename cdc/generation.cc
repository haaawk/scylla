...
constexpr size_t limit_of_streams_in_topology_description() {
...
topology_description limit_number_of_streams_if_needed(topology_description&& desc) {
    ...
    size_t vnode_limit = limit / desc.entries().size();
    auto entries = std::move(desc).entries();
    for (auto& e : entries) {
        if (e.streams.size() > vnode_limit) {
            e.streams.resize(vnode_limit);
        }
    }
